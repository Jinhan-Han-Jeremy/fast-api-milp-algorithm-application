...
class ParallelTeamScheduler:
    ...
    def __init__(self):
        # , tasks: List[str], member_performances: List[Dict[str, Any]]
        self.results = ""  # 최종 결과(문자열 형태)를 저장하기 위함
        self.tasks = []  # 작업(Tasks) 목록
        self.workers = []  # 작업자(Workers) 목록
        self.member_performances = [] # 작업자(Workers)와 퍼포먼스스 목록
    ...
    task_a = "일정과 예산 계획 수립 초기"
    task_b = "팀 구성 및 역할 할당 초기"
    task_c = "프로젝트 킥오프 미팅 주최 초기"
    
    def define_data(self):
        """
        1) 작업, 작업자, 작업 시간 데이터를 정의한다.
           실제로는 외부에서 읽어올 수도 있음.
        """
        self.tasks = [
            "일정과 예산 계획 수립 초기",
        ...
        # 기존 member_performances (집합을 리스트의 딕셔너리로 변환)
        self.member_performances: List[Dict[str, Any]] = [
            {"이프로": 4, "최프로": 3, "이덕마": 3, "최덕마": 3},
            {"최프로": 3, "이덕마": 4, "최덕마": 3, "이비례": 4, "이고례": 4},
            {"이프로": 4, "최프로": 3, "이비례": 4, "최비례": 3, "이고례": 4}
        ]
        
        self.workers = list({worker for performance in self.member_performances for worker in performance.keys()})
    ...
    def get_results_to_pass(self):
        ...
        "durations": self.durations_for_tasks
        }
    
async def run_milp_scheduler():
